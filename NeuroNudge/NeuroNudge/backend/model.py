...
LEXICON = {
    'joy': ['happy', 'joy', 'glad', 'excited', 'delighted', 'pleased', 'good', 'great', 'wonderful', 'awesome', 'fantastic', 'love'],
    'sadness': ['sad', 'down', 'unhappy', 'depressed', 'mourn', 'sorrow', 'gloomy', 'miss', 'isolated', 'lonely', 'disappoint', 'hurt'],
    'anger': ['angry', 'mad', 'furious', 'irritat', 'annoyed', 'rage', 'unfair', 'frustrat', 'outrage', 'pissed'],
    'fear': ['scared', 'afraid', 'anxious', 'nervous', 'worried', 'panic', 'terrif', 'dread'],
...
def predict_emotion(text: str, model=None) -> Tuple[str, float]:
    ...
    if mtype == 'level2' and 'bundle' in model:
        try:
            ...
            if max_conf < 0.4:
                lex_label, lex_score = _simple_lexicon_predict(text)
                if lex_score > 0.5:
                    return lex_label, lex_score
